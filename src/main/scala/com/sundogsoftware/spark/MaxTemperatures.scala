...
//    val minTempRecords = parsedLines.map( x => (x._1, x._3.toFloat))
//    val minTemps=minTempRecords.reduceByKey((x,y) => min(x, y))
//    val maxTempRecords = minTempRecords.reduceByKey((x,y) => max(x, y))
//    val temps = minTemps ++ maxTempRecords
    //temps
    //minTemps.foreach(x => println(f"${x._1} min temperature: ${x._2%.2f}"))
    //maxTempRecords.foreach(x => println(f"${x._1} max temperature: ${x._2%.2f}"))
//
